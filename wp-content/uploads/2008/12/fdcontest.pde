  ...
  for (int i=0; i < lines.length; i++) {
    ...
    if (pieces[0].equals("data"))  {
      ...
      for (int j=2;j<pieces.length;j++)	{
        yValues[dataIndex][j-1]=float(pieces[j]);
      }
...
void draw()	{
  ...
  if (graphType.equals("matrix")) {
    ...
    for (int i=0;i<5;i++) {
      ...
      for (float j=0;j<186;j+=10) {
        ...
        y=plotBottomY[i]-(yValues[int(j)][i+1]/matrixDatamax[i])*(plotBottomY[i]-plotTopY[i]);
      ...
      for (float j=0;j<10;j++) {
        ...
        if(i==4) {
          ...
          text(int(1820+j*20), x, plotBottomY[i]+12);
  ...
  {         
    ...
    textFont(smallFont);
    text("Immigration Act", plotX1+(1924+1965-1821-1821)*.5*(plotX2-plotX1)/186, plotY1+15);
