...
selection=d3.range(data.length).reverse();updateGrid();
...
function updateGrid() {
	var columns=["Nom","Inscrits","Abstention","Blancs","Eva Joly","Marine Le Pen", "Nicolas Sarkozy", "Jean-Luc Mélenchon", "Philippe Poutou", "Nathalie Arthaud", "Jacques Cheminade", "François Bayrou", "Nicolas Dupont-Aignan", "François Hollande"];
	var myGrid=d3.select("#myGrid").html("");myGrid.append("h3").html(selection.length+" lignes sélectionnées.");
	...
	var tr=table.append("tbody").selectAll("tr").data(selection).enter().append("tr");
