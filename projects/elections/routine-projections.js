...
d3.csv("chances.csv",function(csv) {data=csv;
	...
	var hashedR=svg.selectAll(".importantR").data(importantC).enter()
		.append("rect")
		.attr("x",function(d){return 550+20*(d.index%25)+1;})
		.attr("y",function(d){return 50+20*(~~(d.index/25))+1;})
		.attr("id",function(d) {return d.id.replace("c","R");})
		.attr("width",18)
		.attr("height",18)
		.style("fill","url(#hashed)")
	...
	.transition()
		.duration(1000)
			.attr("x",function(d,i) {return 550+20*(i%25)+1;})
			.attr("y",function(d,i) {return 50+20*(~~(i/25))+1;})
			.attr("width",18)
			.attr("height",18)
	...
	svg.append("text").attr("x",550).attr("y",550).text("Les circonscriptions hachurées sont celles où se présente un ministre de François Fillon, un")
	svg.append("text").attr("x",550).attr("y",565).text("proche de Nicolas Sarkozy, ou un ministre de Jean-Marc Ayrault. ")
	...
	function order() {
		if(mode){
			...
			largeRects.transition()
			.attr("x",function(d) {return 550+20*(d.rank%25)+1;})
			.attr("y",function(d) {return 50+20*(~~(d.rank/25))+1;});
			hashedR.transition()
			.attr("x",function(d) {return 550+20*(d.rank%25)+1;})
			.attr("y",function(d) {return 50+20*(~~(d.rank/25))+1;});
		};
