(function() {
	...
	var interface={
		...
		leftPan:{
			applications:["Bandwidth","Percentage of total bandwidth","HD movie equivalent","Sessions consumed","# of apps"],
	...
	var catxy={"networking":[354,166],"media":[620,166],"general-internet":[350,365],"collaboration":[506,375],"business-systems":[748,375]};
	var threatCatxy={
		"adware":[300,166],"backdoor":[450,166],"botnet":[600,166],"code-execution":[750,166],
		"keylogger":[300,375],"net-worm":[412,375],"overflow":[525,375],"spyware":[638,375],"sql-injection":[750,375]
	};
	var techxy={"client-server":[304,166],"browser-based":[570,166],"peer-to-peer":[304,375],"network-protocol":[570,375]};
	var threatxy={"exploit":[404,250],"malware":[600,250]};
	...
	pan.filter = function(criteria) {
		console.log("entering pan-filter")
		...
		if (pan.main=="applications") {
			...
			pan.dataRight[2]=d3.format("4.2%")(d3.max(pan.selection,function(d) {return +d.frequency;})/100)
	...
	pan.computePos=function() {
		...
		//console.log(catsum,catsum1,catsum2,catsum3)
		...
		console.log(techsum,techsum1,techsum2,techsum3)
		...
		yScale["0"]=-200;yScale["unknown-tcp"]=-200,yScale["unknown-udp"]=-200; // shouldn't be required, but can't hurt
		...
		d3.csv(pan.threatfile,function(error,csv) {
			...
			csv.forEach(function(d,index) {
				...
				if (key in globalhash) {
					...
					i=i+1;
					//console.log("creating total for ",key,i)
					globalhash[key]=i;
	...
	pan.drawview = function(m) {
		...
		nodes.forEach(function(d,i) {
			...
			if (pan.strata&&mode<3) {
				...
				nodes[i].y=((d3.scale.linear().range([d.ty-d.max/2,d.ty+d.max/2]).domain([0,5])(+d.risk||0))||d.ty);
			}
			if(nodes[i].r) {
		...
		if(mode==0) {
			...
			psb.append("text").text(pan.main=="applications"?"total bandwidth occupied":"number of detections").attr({x:100,y:262,"text-anchor":"middle"})
		...
		if (mode==3) {
			...
			psb.append("text").text("Frequency: Percent of Installations Where Application Appeared")
				.attr({y:20,x:width/2,"text-anchor":"middle"})
			...
			psb.selectAll(".labelX").data(d3.range(6)).enter().append("text")
				...
				.attr({y:35,x:function(d) {return 115+150*d},"text-anchor":"middle"})}
			psb.selectAll(".labelY").data(pan.main=="applications"?categories:threatCats).enter().append("text")
				.text(String)
				.attr({x:105,y:function(d,i) {return 53+i*(pan.main=="applications"?100:(500/9))},"text-anchor":"end"})
			if(pan.main=="applications") {
			psb.selectAll(".lx").data(d3.range(6)).enter().append("path")
				.attr("d",function(d) {return "M"+(115+150*d)+",45v505"})
				.style("stroke","#eee")}
			psb.selectAll(".ly").data(d3.range(6)).enter().append("path")
				.attr("d",function(d) {return "M115,"+(50+(pan.main=="applications"?100:(500/9))*d)+"h750"})
				.style("stroke","#eee")
		...
		circles.on("mouseover",function(d) {
			...
			if(pan.main=="applications") {
				if(d3.select("#select_dataset").property("value").toLowerCase()=="applications by subcategory") {
					...
					{label:"HD movies equivalent",value:+d.hd},
					{label:"% of total bandwidth",value:d3.format("4.2%")(+d.bandwidth/pan.totalB)},	
					{label:"Sessions consumed",value:+d.sessions},
			...
			var xy=[];
			if(d.x+d.r>600) {
				xy[0]=d3.select("#"+pan.id).property("offsetLeft")+d.x+d.r-250;
			} else {
				xy[0]=d3.select("#"+pan.id).property("offsetLeft")+d.x+d.r+20;
			}
			if(d.y<50) {
				xy[1]=d3.select("#"+pan.id).property("offsetTop")
			} else {
				xy[1]=d3.select("#"+pan.id).property("offsetTop")+d.y+25;
			}
		...
		d3.select(".backrect").on("mouseover",function() {
			console.log("caught by background rect")
	...
	pan.setInterface = function() {
		console.log("now in setInterface")
	...
	pan.view = function(options) {
		...
		d3.select("#select_dataset").property("value","All Applications").on("change",function() {
			console.log("detecting change in select dataset...")
			d3.select("#sidebar").selectAll("a").classed("active",0)
			var v=d3.select(this).property("selectedIndex");
			pan.setInterface();
			if(v<7) {
