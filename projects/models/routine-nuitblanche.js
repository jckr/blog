...
var legend = document.getElementById('legend') && d3.select('#legend')
    .append('svg')
    .attr({
        width: w,
        height: 100
...
function reset() {
    ...
    indexInstall = -1;
    people = [];
    d3.range(params.nbPeople)
        .forEach(function() {
            people.push(generatePeople())
        });
    installs = [];
    d3.range(params.nbAttractions)
        .forEach(function() {
            installs.push(generateInstall())
        });
    ...
    total = [0, 0, 0];
    visitStats = d3.range(params.nbAttractions)
        .map(function(d) {
            return 0;
        })
...
function readForm() {
    ...
    if (h.length) {
        ...
        paramNames.forEach(function(id) {
            if (id in myParams && (!(id in params))) {
                // meaning: it's in the hash, but not yet in the parameters... 
                // in other words, once the page is loaded, the form takes precedence over the hash.
                console.log(id, myParams[id]);
                d3.select('#' + id)
                    .property('value', (id = 'mode' ? myParams[id] : +myParams[id]));
            }
        })
        if (myParams.lyak) {
            d3.select('#lyak')
    ...
    params.night = +d3.select('#night')
        .property('value');
    params.duration = +d3.select('#duration')
        .property('value');
    params.nbPeople = +d3.select('#nbPeople')
        .property('value');
    params.step = +d3.select('#stepAvg')
        .property('value');
    params.stepStd = +d3.select('#stepStd')
        .property('value');
    params.timePerVisit = +d3.select('#timePerVisit')
        .property('value');
    params.timePerVisitO = Math.round(params.timePerVisit / 10);
    params.nbAttractions = +d3.select('#nbAttractions')
        .property('value');
    cScale.domain([0, 10, params.nbAttractions]);
    params.o = +d3.select('#o')
        .property('value');
    params.minCap = +d3.select('#minCap')
        .property('value');
    params.maxCap = +d3.select('#maxCap')
        .property('value');
    params.mode = d3.select('#mode')
        .property('checked');
    ...
    d3.select('#stepStd')
        .on('change', reset);
    d3.select('#timePerVisit')
        .on('change', reset);
    d3.select('#nbAttractions')
        .on('change', reset);
    d3.select('#o')
        .on('change', reset);
    d3.select('#minCap')
    ...
    d3.select('#changeSettings')
        .style('display', function() {
            return hood ? 'block' : 'none';
        })
    d3.select('#switchSettings')
        .on('click', function() {
            hood = !hood;
            d3.select('#changeSettings')
                .style('display', function() {
            ...
            d3.select(this)
                .html(hood ? 'Click to hide model settings' : 'Under the hood');
        })

    params.lyak = d3.select('#lyak')
        .property('checked');
    d3.select('#lyak')
        .on('change', reset);
    ...
    d3.keys(params)
        .forEach(function(param, i) {
            hash = hash + (i ? '&' : '') + param + ' = ' + params[param];
        })
    location.hash = hash;
...
function generatePeople() {
    ...
    return {
        status: (start > 0) ? 8 : 0, // if this person's starting point is after the start of the night their initial status is at hom
        ...
        timeInInstall: 0,
        color: color,
        visited: d3.range(params.nbAttractions)
            .map(function() {
                return 0;
            }),
        id: indexPeople
...
function statusUpdate() {
    statuses.forEach(function(s, i) {
        s.nb = people.filter(function(p) {
                return s.id.indexOf(p.status) > -1;
            })
            .length;
...
function homeDist(p) {
    return Math.hypot(p.x - p.homex, p.y - p.homey);
    // distance to home
}
...
function update() {
    ...
    installs.forEach(function(i) {
        if (!i.outside) {
            if (i.occupancy < i.capacity) { // some room, finally!
                d3.range(d3.min([i.capacity - i.occupancy, i.line.length]))
                    .forEach(function(j) { // we are going to let as many people as we can:
    ...
    people.forEach(function(p) {
        if (tick >= p.start) { // it's not time until it's time.
            p.x = p.stepx;
            p.y = p.stepy; // each params.step  counts. If you moved last tick, it's updated here.
            ...
            if (p.status === 0) { // seeking installation to visit
                ...
                p.visible = 'visible';
                remainingAttr = installs.filter(function(d, j) {
                    return !p.visited[j];
                }) // we compare the installations not yet seen
                if (params.lyak) {
                    remainingAttr.sort(function(i, j) {
                        return (i.line.length) * params.timePerVisit / i.capacity + Math.sqrt(dist(p, i)) / p.speed - Math.sqrt(dist(p, j)) / p.speed - params.timePerVisit * (j.line.length) / j.capacity;
                    })
                    // line length taken into account for choice of attraction.
                    // calculation still naive, can be tweaked.	
                } else {
                    remainingAttr.sort(function(i, j) {
                        return dist(p, i) - dist(p, j);
                    }) // line length not taken into account, only distance.
                }
            ...
            if (p.status === 1) { // going towards an installation
                ...
                var d = dist(p, installs[p.destination]),
                    px = p.x,
                    dx = installs[p.destination].x,
                    py = p.y,
                    dy = installs[p.destination].y;
...
function success() {
    ...
    d3.select('#success')
        .html('The size of the circles is proportional to the number of visitors over the duration of the model.');
    stop();
...
function drawGrid() {
    ...
    var y = d3.scale.linear()
        .range([0, 125])
        .domain([0, params.nbPeople]);
    var x = d3.scale.linear()
        .range([0, 300])
        .domain([0, params.nbAttractions]);

    var installSprites = svg.selectAll('.installs')
        .data([1])
        .enter()
        .append('g')
        .classed('installs', 1)
        .style('clip-path', 'url(#frame)')
        .selectAll('.install')
        .data(installs)
        .enter()
        .append('g')
    ...
    installSprites.append('circle')
        .attr({
            ...
            r: 0 /*function(d) {return Math.sqrt(d.capacity+d.line.length)*/ ,
            class: 'line'
        })
        .style({
            stroke: 'none',
    ...
    installSprites.append('rect')
        ...
        .style('fill', 'black');
    installSprites.selectAll('circle')
        .style('visibility', function(d) {
            return d.outside ? 'hidden' : 'visible'
        }) // installs outside got the rect, so no circle.
    installSprites.selectAll('rect')
        .style('visibility', function(d) {
            return !d.outside ? 'hidden' : 'visible'
        }) // installs inside got the circles, so no rect.

    var peopleSprites = svg.selectAll('.people')
        .data([1])
        .enter()
        .append('g')
        .classed('people', 1)
    ...
    if (!params.small) {
        var monitorChart = svg.selectAll('.mcharts')
            .data([1])
            .enter()
            .append('g')
            .attr('transform', 'translate(550, 50)')
            .classed('mcharts', 1);
        var mCharts = monitorChart.selectAll('path')
            .data(statuses)
            .enter()
            .append('path')
            .classed('mchart', 1)
            .style('stroke', function(d, i) {
                return ['steelblue', 'firebrick', 'limegreen', '#ccc'][i];
            })
            .style('fill', 'none')
            .style('visibility', function(d, i) {
                return i === 2 ? 'visible' : 'hidden'
            })
        var mTexts = monitorChart.selectAll('text')
            .data(statuses)
            .enter()
            .append('text')
            .classed('mText', 1)
        ...
        monitorChart.append('g')
            ...
            .call(d3.svg.axis()
                ...
                .tickSubdivide(1)
                .tickSize(2));
        monitorChart.append('text')
            .text('How many people are visiting an installation ?')
            .classed('dashtitle', 1) //.attr('y', -10)

        var time = svg.append('g')
            .attr('transform', 'translate(550, 0)')
            .classed('time', 1);
        time.append('path')
            .attr('d', 'M48, 12v-2h254v2')
            .style('stroke', 'black')
            .style('fill', 'none');
        time.append('path')
            .classed('cursor', 1)
        ...
        time.append('text')
            ...
            .text(params.night);
        var indics = svg.append('g')
            .attr('transform', 'translate(550, 220)')
            .classed('indics', 1);
        var indic = indics.selectAll('g')
            .data(statuses)
            .enter()
            .append('g')
            .classed('indic', 1)
            .attr('transform', function(d, i) {
                return 'translate(' + 60 * (i + 1) + ', 0)';
            });
        indic.append('text')
            .attr({
                x: 60,
        ...
        itotal.append('text')
            ...
            .text(params.nbPeople)
            .style('font-size', '10px');

        var indBars = indics.selectAll('.indbar')
            .data(total)
            .enter()
            .append('g')
            .classed('indbar', 1)
        ...
        indBars.append('rect')
            .style('fill', function(d, i) {
                return ['steelblue', 'firebrick', 'limegreen'][i];
            })
            .attr('height', 15)
        indBars.append('text')
            .text(function(d, i) {
                return ['Moving', 'Waiting', 'Visiting'][i]
            })
            .attr('y', 25)
            .style({
                'visibility': 'hidden',
        ...
        indics.append('text')
            .classed('dashtitle', 1)
            .text('Globally, everyone spent their time: ')
            .attr({
                x: 0,
                y: 50
            })

        var vStats = svg.selectAll('.vStats')
            .data([1])
            .enter()
            .append('g')
            .classed('vStats', 1)
        ...
        vStats.append('text')
            .text('How many attractions have people visited?')
            .attr('y', -10)
            .classed('dashtitle', 1)
        var vStat = vStats.selectAll('.vStat')
            .data(visitStats)
            .enter()
            .append('g')
            .classed('vStat', 1)
        ...
        vStat.append('rect')
            .attr({
                width: (300 / params.nbAttractions) - 1,
                height: y,
                y: function(d) {
                    return 125 - y(d);
                }
...
function draw() {
    ...
    var installSprites = svg.selectAll('.install')
        .data(installs);
    installSprites.select('.capacity')
        .transition()
        .duration(duration)
        .style('fill-opacity', function(d) {
            return d.occupancy / d.capacity;
        });
    installSprites.select('.line')
        .style('fill', function(d) {
            return d3.scale.linear()
                .range(['palegreen', 'darkgreen', 'orange', 'red'])
                .domain([0, d.capacity / 2, d.capacity, 1.5 * d.capacity])(d.occupancy + d.line.length);
        })
        .transition()
        .duration(duration)
        .attr('r', function(d) {
            return Math.sqrt(d.occupancy + d.line.length);
        })
    ...
    peopleSprites
        ...
        .attr('r', function(d) {
            return (d.status && d.status < 8 && d.visible) ? 2 : 1
        })
    peopleSprites
        // .transition()
        // .duration(duration)
    ...
    d3.select('#timetick')
        .text(tick)
        .attr('x', 50 + 250 * (tick / params.night));
    d3.select('.time')
        .select('.cursor')
        .attr('transform', 'translate(' + (50 + 250 * (tick / params.night)) + ', 10)');
    d3.selectAll('.indic')
        .data(statuses)
        .select('.nb')
        .text(function(d) {
    ...
    d3.selectAll('.indbar')
        .data(total)
        .attr('transform', function(d, i) {
            var a = i ? d3.sum(total.slice(0, i)) : 0;
            a = a * 300 / d3.sum(total);
            return 'translate(' + a + ', 60)';
        });
    d3.selectAll('.indbar')
        .select('rect')
        .attr('width', function(d) {
            return 300 * d / d3.sum(total);
        })
    d3.selectAll('.indbar')
        .select('text')
        .style('visibility', function(d) {
            return d / d3.sum(total) > .125 ? 'visible' : 'hidden';
        })
