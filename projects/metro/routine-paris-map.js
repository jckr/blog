function init() {
  var width = 960,
    height = 800;
  var debugE;
  var svgMap = d3.select('#chart').append('svg').attr('width', width).attr('height', height);
  var myScale = 1;
  var dist, avg;
  ...
  var edgeScale = d3.scale.linear().domain([0, 0.01]).clamp([true]).range([1, 5]);
  ...
  var colors = {};
  d3.keys(lineColors).forEach(function (l) {
    colors[l] = ratpColors[lineColors[l]];
  });
  ...
  var cellSta = svgMap.append('g').attr({width: width, height: height}).attr('id', 'Sta');
  ...
  frame
    .append('rect')
    .attr({width: width - 2, height: height - 2, x: 1, y: 1})
    .style({fill: 'none', stroke: 'white'});

  var selected = -1;
  var data, stations, edges, nodes, edgeArr;
...
function ready(error, s, e, n) {
    ...
    var totalT = d3.sum(s, function (d) {
      return d.trafic;
    });
    ...
    edgesStart = d3
      ...
      .map(e);
    computeDistance(s, e); // factoring walking distance
    ...
    edgesC = e.filter(function (d) {
      if (['entrance', 'exit', 'correspondance'].indexOf(d.type) === -1) {
        d.color = ratpColors[lineColors[d.type]];
        return true;
      }
    });
    ...
    g3.append('circle')
      .attr('class', 'staCircles')
      .style({
        fill: function (d, i) {
          return timeScale(dist[i].avg);
        },
        stroke: 'black',
      })
    ...
    d3.selectAll('.staCircles').on('click', function (d) {
    ...
    cellSta
      ...
      .style({
        'stroke-width': function (d) {
          return edgeScale(d.share);
        },
        'stroke-dasharray': function (d) {
          return d.type === 'marche' ? '2 2' : null;
        },
        stroke: function (d) {
          return e[d.id].active ? d.color : 'black';
        },
        opacity: function (d) {
          return d.type === 'marche' ? 0 : 1;
        },
      })
      .on('click', function (d) {
        ...
        edgesStart = d3
          ...
          .map(e);
        dist = calcDist(n, edgesStart, s);
        d3.selectAll('.staCircles').style({
          fill: function (d, i) {
            return timeScale(dist[i].avg);
          },
          stroke: 'black',
        });
        reinit(geoCheck, n, e, s, s[selected], dist);
      });
    d3.selectAll('.lignes')
      .append('title')
      .text(function (d) {
        return e[d.id].active ? 'actif' : 'désactivé';
      });
    ...
    var legend1 = legend
      .append('g')
      .attr({id: 'legend1', transform: 'translate(670,0)', 'text-anchor': 'end'})
      .style('opacity', 0);
    legend1
      .append('text')
      .attr({y: 15, x: 285})
      .text('Seuls les chemins les plus courts sont affichés.');
    legend1.append('text').attr({y: 40, x: 285}).text('Un anneau = 5 minutes de trajet');
    legend1.append('text').attr({x: 215, y: 65}).text('Marche');
    legend1
      .append('path')
      .attr({d: 'M285,65h-60'})
      .style({stroke: '#222', 'stroke-dasharray': '2 2'});
  ...
  function reticulate(edges, stations) {
    ...
    edges.forEach(function (e) {
      ...
      if (!e.o1) {
        e.o1 = e.p1;
        e.o2 = e.p2; // original positions
      }
      e.path = 'M' + e.p1.join(',') + 'L' + e.p2.join(',');
    });
  }
  
  function reinit(geoCheck, nodes, edges, stations, s, dist) {
    ...
    repos(nodes, edges, stations, s, dist);
  }
  ...
  function present(seconds) {
    ...
    var t =
      Math.floor(seconds / 60) + ' minutes ' + (s ? s + (s > 1 ? ' secondes' : ' seconde') : '');
    return t;
  }
  ...
  function repos(nodes, edges, stations, s, dist) {
    ...
    if (!s) {
      ...
      d3.select('em').html(
        'Cliquez sur une station pour voir les temps de trajet, cliquez sur un tronçon pour le désactiver'
      );
      stations.forEach(function (d) {
        d.projx = d.ox;
        d.projy = d.oy;
      ...
      var x0 = s.ox,
        y0 = s.oy,
        maxDist = d3.max(
          dist[s.station].distances.filter(function (d) {
            return d < 100000;
          })
        );
      ...
      var solution = dist[s.station].edges.map(function (d) {
        return d.id;
      });
      ...
      d3.selectAll('.staCircles').on('mouseover', function (d) {
        ...
        if (d.main != s.main) {
          ...
          d3.selectAll('.staCircles')
            .filter(function (d) {
              return corrs.indexOf(d.station) < 0;
            })
            .transition()
            .style('opacity', 0);
          d3.selectAll('.lignes')
            ...
            .attr('d', function (d) {
              return d.path;
            }) // this is necessary to repeat b/c in case of accidental mouseover during transformation, the edges would stop moving
            .style('opacity', function (l) {
              if (solution.indexOf(l.id) > -1) {
                // edge to be displayed
                if (path.indexOf(l.id) > -1) {
                  // edge to be displayed AND in shortest path
                  return 1;
                } else {
                  return 0.1; // edge not in shortest path from center to highlighted point
                }
              } else {
                return 0;
              } // edge not to be displayed
            });
        }
        d3.select('em').html(
          'Temps vers ' + d.name + ' : ' + present(dist[s.station].distances[d.station])
        );
      });
  ...
  function calcDist(nodes, edges, stations) {
    ...
    stations.forEach(function (s) {
      myDist = dijkstra(nodes, edges, s.main);
      dist.push({
        distances: myDist.distances.slice(381, 680),
        edges: myDist.edges,
        avg: d3.mean(myDist.distances),
      });
    });
  ...
  function dijkstra(nodes, edges, s) {
    var dist = d3.range(nodes.length).map(function () {
      return Infinity;
    });
    var backtrack = [];
  
    dist[s] = 0;
    var Q = new BinaryHeap(function (v) {
      return dist[v];
    });
  ...
  function lowerPart(station, nodes, edges) {
    if (!station) {
      d3.select('#r4').style('display', 'none');
      return true;
    }
    ...
    d3.select('#slegend').html('&nbsp;');
    d3.select('#clegend').html('&nbsp;');
    ...
    liSSVG
      .append('rect')
      .attr({height: 24, rx: 3, ry: 3, width: 36})
      .style('fill', function (d) {
        return colors[nodes[d.end].ligne];
      });
    liSSVG
      .append('text')
      .attr({x: 18, 'text-anchor': 'middle', y: 18})
      .text(function (d) {
        return nodes[d.end].ligne;
      });
    ...
    liSSVGg
      ...
      .style({fill: '#eee', stroke: 'black', 'stroke-width': 2});
    liSSVGg
      .selectAll('path')
      .data(d3.range(4))
      .enter()
      .append('path')
      .attr('d', function (d) {
        return 'M' + 50 * (d + 1) + ',2v20';
      })
      .style({stroke: '#bbb', 'stroke-dasharray': '4 4'});
    liSSVGg
      .append('rect')
      .attr({
        y: 6,
    ...
    liSSVGg
      .append('rect')
      .attr({width: 250, x: 2, y: 2, height: 20})
      .style({fill: 'white', opacity: 0.0001, stroke: 'none'});
    ...
    liCSVG
      ...
      .attr({height: 24, rx: 3, ry: 3, width: 36})
      .style('fill', function (d) {
        return colors[nodes[d.start].ligne];
      });
    liCSVG
      .append('text')
      .attr({x: 18, 'text-anchor': 'middle', y: 18})
      .text(function (d) {
        return nodes[d.start].ligne;
      });
    liCSVG
      .append('rect')
      .attr({x: 45, height: 24, rx: 3, ry: 3, width: 36})
      .style('fill', function (d) {
        return colors[nodes[d.end].ligne];
      });
    liCSVG
      .append('text')
      .attr({x: 63, 'text-anchor': 'middle', y: 18})
      .text(function (d) {
        return nodes[d.end].ligne;
      });
