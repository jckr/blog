...
void mouseOver(int i, int y, float x1,float x2,float y1, float y2){
  ...
  if (i==1){text((y+1980)+": Contribution rate: "+nfs(int(100*myTauxCot[y]),0,0)+"%, total revenue: "+int(myReceipts[y]/1000)+" billion euros",x1+5,y1+5);}
  if (i==2){text((y+1980)+": Replacement rate: "+nfs(int(100*myTauxRempl[y]),0,0)+"%, total spending: "+int(mySpending[y]/1000)+" billion euros",x1+5,y1+5);}
  if (i==3){text((y+1980)+": System balance: "+int(myBalance[y]/1000)+" milliards d'euros",x1+5,y1+5);}
}
