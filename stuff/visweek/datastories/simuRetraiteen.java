...
public class simuRetraiteen extends PApplet {
...
public void setup() {
  ...
  size(800,600);
  title[0]="Retirement age";
  boxCoords[0][0][0]=20;
  boxCoords[0][0][1]=20;
  boxCoords[0][1][0]=580;
  boxCoords[0][1][1]=380;

  title[1]="Contribution rate";
  boxCoords[1][0][0]=600;
  boxCoords[1][0][1]=20;
  boxCoords[1][1][0]=780;
  boxCoords[1][1][1]=190;

  title[2]="Replacement rate";
  boxCoords[2][0][0]=600;
  boxCoords[2][0][1]=210;
  boxCoords[2][1][0]=780;
  boxCoords[2][1][1]=380;
...
public void mouseOver(int i, int y, float x1,float x2,float y1, float y2){
  int boxX=0, boxY=0;
  fill(0);
  if (i==0){text((y+1980)+": Retirement age: "+nfs(myRetirementAge[y],0,0)+", employee per pensioner: "+nfs(myEmployees[y]/popRetired[y],1,1),x1+5,y1+5-((boxMode[i]!=1)?20:0));}
  if (i==1){text((y+1980)+": Contribution rate: "+nfs(PApplet.parseInt(100*myTauxCot[y]),0,0)+"%, total revenue: "+PApplet.parseInt(myReceipts[y]/1000)+" billion euros",x1+5,y1+5);}
  if (i==2){text((y+1980)+": Replacement rate: "+nfs(PApplet.parseInt(100*myTauxRempl[y]),0,0)+"%, total spending: "+PApplet.parseInt(mySpending[y]/1000)+" billion euros",x1+5,y1+5);}
  if (i==3){text((y+1980)+": System balance: "+PApplet.parseInt(myBalance[y]/1000)+" milliards d'euros",x1+5,y1+5);}
}
...
class Integrator {
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--present", "--bgcolor=#666666", "--stop-color=#cccccc", "simuRetraiteen" });
  }
}
